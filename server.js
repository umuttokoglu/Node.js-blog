...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('publishPost', function (data) {
    var dbData = {"user_id":1, "post_head": data.post_head, "category_id":data.category_id, "post_content": data.post_content, "post_publish":data.post_publish};
  ...
  socket.on('updatePost', function (data) {
    var sql = 'Update post set post_head="'+data.post_head+'",category_id='+data.category_id+',post_content="'+data.post_content+'",post_publish='+data.post_publish+' where id='+data.id;
    conn.query(sql, function (err, req) {
      if (!err) {
        console.log(data.id + ' id\'li makale başarıyla güncellendi.');
      }
      else{
        console.log(err);
      }
  ...
  socket.on('retriveforDelete', function (data) {
    if (data) {
      conn.query('Select id, post_head, post_date from post' , function (err, req) {
        if (!err) {
  ...
  socket.on('readMore', function (data) {    
    conn.query('Select * from post where id=?', data, function (err, req) {
      if (!err) {
        socket.emit('readMorePage', req);
      }
      else{
        console.log(err);
      }
  ...
  socket.on('search', function (data) {
    var sql = 'Select * from post where post_head like "%'+data+'%"';
    conn.query(sql, function (err, req) {
      if (!err) {
        socket.emit('searchResult',req);
      }
      else{
        socket.emit('searchResult',0);
      }
    });
  });
});
...
app.put('/api/post/:id',(req, res) => {
  let sql = 'Update post set post_head="'+req.body.post_head+'",post_content="'+req.body.post_content+'" where id='+req.params.id;
  console.log(sql);
  let query = conn.query(sql, (err, results) => {
    if(err) throw err;
    res.send(JSON.stringify({"status": 200, "error": null, "response": results}));
